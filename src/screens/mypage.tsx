...
export function MyPageScreen() {
  const { user, activeQrSession, deviceSimulation } = useAppContext();
...
export function MyStatsScreen() {
  ...
  const monthlyData = [
    { label: '1월', mine: 2, avg: 1.5 },
    { label: '2월', mine: 3, avg: 2.0 },
    { label: '3월', mine: 1, avg: 1.8 },
    { label: '4월', mine: 4, avg: 2.2 },
    { label: '5월', mine: 2, avg: 2.5 },
    { label: '6월', mine: 5, avg: 2.8 },
  ];

  const currentData = period === '3months' ? monthlyData.slice(-3) : monthlyData;
  const averageMine = (currentData.reduce((sum, item) => sum + item.mine, 0) / currentData.length).toFixed(1);
  const averageAll = (currentData.reduce((sum, item) => sum + item.avg, 0) / currentData.length).toFixed(1);
...
export function MySharesScreen() {
  ...
  return (
    <AppScreen scroll contentContainerStyle={styles.pageContent}>
      ...
      <View style={styles.highlightPanel}>
        <Text style={styles.highlightPanelTitle}>총 나눔 12회</Text>
        <Text style={styles.highlightPanelText}>여러분의 작은 나눔이 누군가에게 큰 힘이 되고 있어요.</Text>
      </View>

      <View style={styles.resultList}>
        {mockShareHistory.map((item) => (
          <Pressable
            key={item.id}
