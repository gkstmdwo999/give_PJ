...
export const chatAPI = {
  ...
  async listMessages(chatRoomId: string, authToken?: string, viewerId = ''): ApiResult<ChatMessage[]> {
    const response = await requestEnvelope<{ messages?: any[] } | any[]>(
      `${backendConfig.endpoints.chats}/${chatRoomId}/messages`,
      {
        headers: buildAuthHeaders(authToken),
      },
    );
  ...
  async sendMessage(payload: {
    ...
    const response = await safeFetch<{ data?: ChatMessageRecord }>(
      `/chat-rooms/${payload.chatRoomId}/messages`,
      {
        method: 'POST',
        headers: withJsonHeaders({ 'Content-Type': 'application/json' }),
        body: JSON.stringify({
...
export const reviewAPI = {
  ...
  async create(payload: Omit<ReviewRecord, 'reviewId' | 'createdAt'>): ApiResult<ReviewRecord> {
    return {
      data: {
        ...payload,
...
export const postAPI = {
  ...
  async checkHarmfulItem(
    ...
    const response = await requestEnvelope<ImageAnalysisResult>(backendConfig.endpoints.harmfulCheck, {
      method: 'POST',
