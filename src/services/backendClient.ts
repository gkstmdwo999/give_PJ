...
export const backendConfig = {
  ...
  endpoints: {
    ...
    authSignup: process.env.EXPO_PUBLIC_AUTH_SIGNUP_PATH ?? '/api/auth/signup',
    authLogout: process.env.EXPO_PUBLIC_AUTH_LOGOUT_PATH ?? '/api/auth/logout',
    posts: process.env.EXPO_PUBLIC_POSTS_PATH ?? '/api/posts',
    harmfulCheck: process.env.EXPO_PUBLIC_HARMFUL_CHECK_PATH ?? '/api/posts/check-harmful',
...
function mapBackendRole(role?: string): RoleCode {
  if (role === 'ADMIN' || role === 'BENEFICIARY' || role === 'USER') {
    return role;
  }
  return 'USER';
}
...
export function mapBackendUser(raw: any): User {
  const roleCode = mapBackendRole(raw?.roleCode ?? raw?.roleName);
  const location = locationFromUnknown(raw?.location, raw?.dongName);
  const vulnerableTypes = Array.isArray(raw?.vulnerableTypes) ? raw.vulnerableTypes : [];

  return {
    id: String(raw?.id ?? raw?.memberId ?? raw?.userId ?? `user_${Date.now()}`),
    roleId: String(raw?.roleId ?? `role_${roleCode.toLowerCase()}`),
    roleCode,
    name: raw?.name ?? '사용자',
    nickname: raw?.nickname ?? raw?.name ?? '사용자',
    email: raw?.email ?? '',
    phone: raw?.phone ?? '',
    dongName: raw?.dongName ?? location.dongName,
...
export function mapBackendPost(raw: any, fallbackLocation?: NeighborhoodLocation): Post {
  ...
  const location = locationFromUnknown(raw?.location, fallbackLocation?.dongName);

  return {
    id: String(raw?.id ?? raw?.postId ?? raw?.recordId ?? `post_${Date.now()}`),
    recordId: String(raw?.id ?? raw?.postId ?? raw?.recordId ?? `post_${Date.now()}`),
    type,
    title: raw?.title ?? '',
...
export function mapBackendChatRoom(raw: any, viewerId: string): ChatRoom {
  ...
  return {
    id: String(raw?.id ?? raw?.chatId ?? raw?.chatRoomId ?? `chat_${Date.now()}`),
    roomStatus: raw?.roomStatus === 'closed' ? 'closed' : 'open',
    donorId: String(raw?.donorId ?? ''),
    requesterId: String(raw?.requesterId ?? viewerId),
    userId: String(participant?.id ?? raw?.userId ?? ''),
    ...
    postId: raw?.postId,
    postType: raw?.postType === 'need' ? 'need' : raw?.postType === 'share' ? 'share' : undefined,
...
export function mapBackendChatMessage(raw: any, viewerId: string): ChatMessage {
  const senderId = String(raw?.sender?.id ?? raw?.senderId ?? '');
